...
resource "aws_security_group" "lb-sg" {
  ...
  name        = "lb-sg"
  description = "Allow HTTP/HTTPS traffic to Jenkins SG"
...
resource "aws_security_group" "jenkins-sg" {
  ...
  ingress {
    description     = "allow anyone on port ${var.webserver-port}"
  ...
  ingress {
    description = "allow all traffic from us-west-2 network"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
...
resource "aws_security_group" "jenkins-sg-oregon" {
  ...
  ingress {
    description = "Allow all traffic from us-east-1 network"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
